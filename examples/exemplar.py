#!/usr/bin/env python3
"""
examples/exemplar.py - Syncbot exemplar mock entry point.
"""
...
def _log_targets() -> list[str]:
    log_root = _resolve_any(("LOGS", "LOG_PATHS"), ("LOGS", "LOG_PATHS"), ".logs/Syncbot/")
    ...
    if not out:
        out = [str(log_root / "Syncbot_SESSION.log"), str(log_root / "Syncbot_MASTER.log")]
    return out
...
def draw_header() -> None:
    title = f"{CONFIG.get('INFO', {}).get('PROJECT_NAME', 'Syncbot')} Exemplar"
    try:
        logger.apply_border(
            text=title,
...
def run_module_demo() -> None:
    ...
    from Syncbot.bootstrap import bootstrap_syncbot_config_loader, resolve_syncbot_config_path

    resolved = resolve_syncbot_config_path(anchor_file=__file__)
    log_line(f"resolved config path: {resolved}", key="!done")

    try:
        bootstrap_syncbot_config_loader(anchor_file=__file__)
        log_line("bootstrap_syncbot_config_loader executed", key="!done")
    except Exception as exc:
        log_line(f"bootstrap skipped: {exc}", key="!warning")
...
def run_edge_cases() -> None:
    ...
    resolved_default = resolve_syncbot_config_path(anchor_file=None)
    resolved_this = resolve_syncbot_config_path(anchor_file=__file__)

    log_line(f"resolve(None) -> {resolved_default}", key="!data")
    log_line(f"resolve(__file__) -> {resolved_this}", key="!data")
